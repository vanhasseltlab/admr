...
test_that("timedIRMC works with examplomycin data", {
  ...
  # Run timedIRMC
  result <- timedIRMC(init, opts, obs)
...
test_that("timedIRMC handles multiple models correctly", {
  test_data <- create_test_data()
  ...
  # Run timedIRMC with multiple models
  result <- timedIRMC(init, opts, obs, nomap = FALSE)
...
test_that("timedIRMC handles boundary constraints", {
  test_data <- create_test_data()
  opts <- test_data$opts
  obs <- test_data$obs
...
test_that("timedIRMC handles iteration history", {
  test_data <- create_test_data()
  opts <- test_data$opts
  obs <- test_data$obs
...
test_that("timedIRMC handles convergence", {
  test_data <- create_test_data()
  opts <- test_data$opts
  obs <- test_data$obs
  ...
  # Check convergence
  expect_true(min(result$nll) < result$nll[1])
  expect_true(all(diff(result$nll[1:3]) <= 0))  # First 2 iteration should show improvement
})
