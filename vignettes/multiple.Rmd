...
The `fitIRMC` function supports multiple chains to improve optimization:
...
result <- fitIRMC(
  ...
  pertubation = 0.1  # Perturbation factor
)
...
You can customize the optimization phases:

```{r, eval=FALSE}
# Custom phase fractions
result <- fitIRMC(
  opts = opts,
  obs = examplomycin_aggregated,
  phase_fractions = c(0.3, 0.3, 0.2, 0.2)  # Custom phase distribution
)
```
...
rxModel2 <- function(){
  ini({
    cl <- 4.958792  # Clearance
    v1 <- 10.155736 # Volume of central compartment
    v2 <- 29.998821 # Volume of peripheral compartment
    q <- 9.826382   # Inter-compartmental clearance
    ka <- 1.027525  # Absorption rate constant
...
```{r}
predder <- function(time, theta_i, dose = 200) {
...
pred <- predder(time_points, theta_i)

pred_df <- data.frame(
  time = rep(time_points, each = n_subjects),
  concentration = as.vector(pred),
  sim.id = rep(1:n_subjects, times = length(time_points)))

percentiles_df <- pred_df %>%
  group_by(time) %>%
  summarize(
    median = median(concentration),
    lower = quantile(concentration, 0.05),
    upper = quantile(concentration, 0.95)
  ) %>%
  ungroup()
...
dosing_plot <- ggplot() +
  ...
  geom_ribbon(data = percentiles_df, 
              aes(x = time, ymin = lower, ymax = upper), 
              fill = "#cce5df", alpha = 0.6) +
  
  # Median line
  geom_line(data = percentiles_df, 
            aes(x = time, y = median), 
