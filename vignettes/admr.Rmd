...
```{r}
opts <- genopts(g=function(beta,bi=rep(0,length(beta)),ai) {beta*exp(bi)},
                  xt=c(.1,.25,.5,1,2,3,5,8,12),
                  p=list(beta=c(cl = 5, v1 = 10, v2 = 30,q = 10, ka = 1),Omega=omegas(.09,0,5),Sigma_prop=0.04),
...
fit <- fitEM(opts$pt, opts, examplomycin_aggregated, chains = 5)
```
```{r}
print(fit)
```
