...
p_to_optim <- function(p) {
  ...
  if (any(names(p)=="Sigma_exp")) {
    ...
    if (any(names(p)=="Sigma_prop")) p$Sigma_prop <- p$Sigma_prop+p$Sigma_exp else p$Sigma_prop <- p$Sigma_exp
    p$Sigma_exp <- NULL
  }
  p2 <- map(p,~if(is.list(.) | is.matrix(.)) {p_transform_mat(.)} else {p_transform_vec(.)})
  p3 <- map(p2,~.[[1]])
  ...
  list(values=valuess,backtransformfunc=backtransformfunc,d_psi_d_psitrans_long=d_psi_d_psitrans_long,
       d_psi_d_psitrans_short=d_psi_d_psitrans_short,
       d_bi_d_omega=p2$Omega$d_bi_d_omega,d_omega_d_Omega=p2$Omega$d_omega_d_Omega)
}
